...
normalize_counts_for_sample_PSI <- function(df, sample_name, anchor_genes_list){
  ...
  base::print(paste(sample_name, "swapping_rate:", swapping_rate))
...
for (sample_tmp in unique_sample_names){
  ...
  for (filepath in sample_filepaths) {
    ...
    tmp_to_ref_PSI <- tmp_to_ref %>% 
      ...
      filter((mode == "INCLUDED") | (mode == "SKIPPED" & offset == "0")) %>%
      group_by(index) %>%
      ...
      # select(-total_sum)
      filter(mode == "SKIPPED") %>% 
      mutate(included = total_sum - count) %>%
      mutate(skipped = count) %>%
      mutate(count = included) %>%
      select(-total_sum, -included)
      
    # Normalize this file by the anchor genes.
    tmp_to_ref_PSI <- normalize_counts_individual(tmp_to_ref_PSI, top_normalization_seq_index)
