...
all_sample_reps <- fread("/broad/dawnccle/processed_data/reprocess_250221/count_normalized_v4_merged/MUT_all_samples_raw_counts.csv")
all_sample_reps <- all_sample_reps %>% 
  filter(mode == "INCLUDED") %>% 
  mutate(index_offset = paste0(index, "__", offset)) 


all_event_pairs <- fread("/broad/dawnccle/processed_data/reprocess_250221/count_normalized_v4_merged/MUT_included_all_unique_event_pairs.csv") %>% 
  select(-index) %>% 
...
get_paradise_output <- function(df, all_event_pairs, condition1, condition2) {
  ...
  unique_samples_condition1 <- df %>% 
    filter(grepl(condition1, condition)) %>% 
    pull(sample) %>% 
    unique()
  ...
  if (condition2 == "all") {
    ...
    unique_samples_condition2 <- df %>% 
      filter(grepl(condition2, condition)) %>% 
      pull(sample) %>% 
      unique()
  }
