...
all_sample_reps <- fread("/broad/dawnccle/processed_data/reprocess_250221/count_normalized_v4_merged/MUT_all_samples_PSI_count_table.csv")
...
get_paradise_output <- function(df, condition1, condition2) {
  ...
  condition1_df <- df %>% filter(grepl(condition1, condition)) %>% mutate(PSI = included_count / (included_count + skipped_count))
  condition2_df <- df %>% filter(grepl(condition2, condition)) %>% mutate(PSI = included_count / (included_count + skipped_count))
