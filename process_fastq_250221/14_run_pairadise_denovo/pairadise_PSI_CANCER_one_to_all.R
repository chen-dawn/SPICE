...
design_target <- args[4]

print(paste("Celltype 1: ", celltype1))
print(paste("Celltype 2: ", celltype2))
print(paste("Output filename: ", output_filename))
print(paste("Design target: ", design_target))
all_sample_reps <- fread("/broad/dawnccle/processed_data/reprocess_250221/count_normalized_denovoR2/denovoR2_all_samples_PSI_count_table.csv")
all_sample_reps <- all_sample_reps %>% 
  filter(grepl("CANCERspecific", condition)) %>% 
    # Filter for deigns that are in the design target
  filter(grepl(design_target, design)| grepl("design0", design)) %>% 
