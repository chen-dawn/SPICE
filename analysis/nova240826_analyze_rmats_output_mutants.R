...
p1 <- ggplot(RBM5_all_df, aes(x = PSI_diff, y = -log10(FDR), color = is_top_gene)) + 
  geom_point() + 
...
ggsave("~/Dropbox (Harvard University)/02Splicing/SplicingManuscript/figure_outputs/RBM5_volcano_plot.png", p1, width = 10, height = 10, units = "in", dpi = 300)


# Also do for RBM10
RBM10_all_df <- fread("/Volumes/broad_dawnccle/processed_data/missplicing_processed_df/novaseq240826/pairadise_indiv_PSI/MUT-sgCh3-1_MUT-sgRBM10/MUT-sgCh3-1_MUT-sgRBM10_rMATS_Result_P.FDR.txt")

RBM10_all_df <- RBM10_all_df %>% 
  mutate(
    PSI1 = mapply(calculate_ratio, I1, S1),
    PSI2 = mapply(calculate_ratio, I2, S2)
  ) %>% 
...
# Plot with ggplot2 and ggrepel
p2 <- ggplot(RBM10_all_df, aes(x = PSI_diff, y = -log10(FDR), color = is_top_gene)) + 
  geom_point() + 
  geom_vline(xintercept = c(-0.2, 0.2), linetype = "dashed") + 
  geom_hline(yintercept = -log10(0.01), linetype = "dashed") + 
  geom_text_repel(aes(label = top_genes), na.rm = TRUE) + 
  scale_color_manual(values = c("top_gene" = "red", "other" = "grey")) +
  theme_bw() + 
  ggtitle("Control sgRNA vs RBM10 KO")
ggsave("~/Dropbox (Harvard University)/02Splicing/SplicingManuscript/figure_outputs/RBM10_volcano_plot.png", p2, width = 10, height = 10, units = "in", dpi = 300)

# Also do for RUBP1
RUBP1_all_df <- fread("/Volumes/broad_dawnccle/processed_data/missplicing_processed_df/novaseq240826/pairadise_indiv_PSI/MUT-sgCh3-1_MUT-sgRUBP1/MUT-sgCh3-1_MUT-sgRUBP1_rMATS_Result_P.FDR.txt")

RUBP1_all_df <- RUBP1_all_df %>% 
  mutate(
    PSI1 = mapply(calculate_ratio, I1, S1),
    PSI2 = mapply(calculate_ratio, I2, S2)
  ) %>% 
...
RUBP1_all_df <- RUBP1_all_df %>%
  ...
  # Set the one where p-value = 0 to be 1e-10.
  mutate(FDR = ifelse(FDR == 0, 1e-10, FDR)) %>%
  # mutate(top_genes = ifelse((log2(PSI_ratio) > 1 | log2(PSI_ratio) < -1) & (-log10(FDR) > -log10(0.01)), gene_name, NA)) %>%
  mutate(is_top_gene = ifelse(!is.na(top_genes), "top_gene", "other"))

# Plot with ggplot2 and ggrepel
p3 <- ggplot(RUBP1_all_df, aes(x = PSI_diff, y = -log10(FDR), color = is_top_gene)) + 
  geom_point() + 
  geom_vline(xintercept = c(-0.2, 0.2), linetype = "dashed") + 
  geom_hline(yintercept = -log10(0.01), linetype = "dashed") + 
  geom_text_repel(aes(label = top_genes), na.rm = TRUE) + 
  scale_color_manual(values = c("top_gene" = "red", "other" = "grey")) +
  theme_bw() + 
  ggtitle("Control sgRNA vs FUBP1 KO")

# Last one is MUT-plx317_U2AF1_WT_MUT-plx317_U2AF1_Q157A
U2AF1_all_df <- fread("/Volumes/broad_dawnccle/processed_data/missplicing_processed_df/novaseq240826/pairadise_indiv_PSI/MUT-plx317_U2AF1_WT_MUT-plx317_U2AF1_Q157A/MUT-plx317_U2AF1_WT_MUT-plx317_U2AF1_Q157A_rMATS_Result_P.FDR.txt")

U2AF1_all_df <- U2AF1_all_df %>% 
  mutate(
    PSI1 = mapply(calculate_ratio, I1, S1),
    PSI2 = mapply(calculate_ratio, I2, S2)
  ) %>% 
...
# Plot with ggplot2 and ggrepel
p4 <- ggplot(U2AF1_all_df, aes(x = log2(PSI_ratio), y = -log10(FDR), color = is_top_gene)) + 
  geom_point() + 
  ...
  scale_color_manual(values = c("top_gene" = "red", "other" = "grey")) +
  theme_bw() + 
  ggtitle("Overexpression U2AF1 WT vs U2AF1 Q157A")

g <- arrangeGrob(p2, p3, p4, p1, ncol = 2)

ggsave("~/Dropbox (Harvard University)/02Splicing/SplicingManuscript/figure_outputs/KO_volcano_plots.png", g, width = 10, height = 6, units = "in")

g <- arrangeGrob(p3, p1, p2,ncol = 3)
ggsave("~/Dropbox (Harvard University)/02Splicing/SplicingManuscript/figure_outputs/KO_volcano_plots_PSI_diff.pdf", g, width = 20, height = 4, units = "in")
...
combined_psi_filtered <- combined_psi %>% 
  ...
  filter(count_sum_average2 > 30) %>% 
  mutate(log2_PSI_ratio = log2(PSI_ratio), log2_PSI_reverse_ratio = log2(PSI_reverse_ratio))
write_csv(combined_psi, "combined_3ss_filtered.csv")
