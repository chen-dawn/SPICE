...
all_sample_reps <- fread("/broad/dawnccle/processed_data/missplicing_processed_df/V5/all_sample_reps_PSI.csv") %>% 
  filter(condition != "Kelly_old")


# # Extract unique lineages
# unique_lineages <- tissue_metadata %>% 
#   select(lineage) %>%
#   distinct()
# 
# # Write unique lineages to a text file. 
# write.table(unique_lineages, file = "/Volumes/broad_dawnccle/melange/data/unique_lineages.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)

print("Finished reading data")

get_paradise_output <- function(df, condition1) {
...
formatted_df <- get_paradise_output(all_sample_reps, celltype1)
