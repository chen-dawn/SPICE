...
# Plot heatmap for the shortlist_genes and add the tau values. Save the heatmap to a file.
pdf("~/Dropbox (Harvard University)/02Splicing/SplicingManuscript/figure_outputs/heatmap_RBP_tau.pdf", width = 20, height = 8)
...
calculate_correlation_for_exon <- function(seq) {
  ...
  correlations <- map_dfr(gene_list, function(gene) {
    ...
    if (max(gex_values, na.rm = T) - min(gex_values, na.rm = T) < 1) {
      return(tibble(Gene = gene, Exon = seq, Correlation = NA))
    }
    corr_val <- cor(gex_values, psi_values, use = "pairwise.complete.obs", method = "pearson")
...
high_correlations <- correlation_df %>% filter(abs(Correlation) > 0.5) %>% 
  group_by(Gene) %>% 
  summarise(num_high_correlations = n()) %>% 
  arrange(desc(num_high_correlations)) 

# Get the high correlations for the top 20.
top_correlations <- correlation_df %>% filter(Gene %in% high_correlations$Gene) %>% 
...
for (i in 1:nrow(top_correlations)) {
  ...
  p <- ggplot(plot_df, aes(x = gex_values, y = psi_values, color = cell_line)) +
    geom_point() +
    theme_minimal() +
    labs(x = "Gene expression", y = "log2(Inclusion/Exclusion)") +
    ggtitle(paste(gene, seq)) + 
    # Add subtitle with the gene name and sequence name
    theme(plot.title = element_text(size = 10, face = "bold"),
          plot.subtitle = element_text(size = 8, face = "italic")) + 
...
# Write the top correlations to a file
write_csv(top_correlations, "~/Dropbox (Harvard University)/02Splicing/test_psi_high_corr_rmats_sig_seq_first/top_correlations.csv")
